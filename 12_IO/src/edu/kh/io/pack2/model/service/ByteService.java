...
public class ByteService {
	 ...
	 * 
	 * Byte 기반 스트림
	 * - 1byte 단위를 데이터를 입/출력 하는 스트림
	 * - 최상위 클래스 : InputStream, OutputStream
	 * - 문자열, 이미지, 영상, 오디오, pdf 등
	 * 모든것을 입/출력 할 수 있음
	 * (단, 통로가 1byte 로 좁다보니 속도가 좀 느림
	 * char 형 문자같은 경우 깨질 우려가 있음)
	 * 
	 * 
	 * */
	
	// IO 관련된 코드 : IOExcepction 발생될 가능성이 높다
	// -> IO 관련 코드는 거의 모든 메서드가 throws IOException
	// -> IOEception 예외처리 구문을 작성하는 것이 필수!
	
	/**
	 * 바이트 기반 스트림을 이용한 파일 출력
	 * + 데이터 출력
	 */
	public void fileBytrOutput() {
		
		// FileOutputSteam 참조변수 선언(OutputStream 상속받은 자식 클래스)	
		FileOutputStream fos = null;
		
		try {
			// new FileOutputStream("경로")
			// -> 경로에 작성도니 파일과 연결된 파일 출력 스트림 객체 생성
			...
			sb.append("!@#$%^&*()_+{}<>\n");
			sb.append("가나다라마바사\n");
			sb.append("자바는 새로운 것이 많아 배우기 어렵다\n");
			sb.append("열심히 공부 해야 한다\n");
			sb.append("정말로 열심히 하겠습니다\n");
			
			// 출력방법 1 : 한 글자(2byte)씩 파일로 출력
			
			// StringBuilder -> String 변환 -> charAt() -> char 로 변환
			...
			fos.write(content.getBytes()); // byte[] 만들어 반환
		...
		} finally { // try 에서 예외발생 여부 관계 없이 무조건 수행
			...
			try {
				...
				// close() 메서드도 IOException 을 발생시키기 때문에
	...
	/**
	 * 버퍼를 이용한 파일 출력
	 * BufferdeFileOutput 보조 스트림 이용
	 */
	public void bufferdeFileOutput() {
		/*
		 * 버퍼(buffer)란?
		...
		try {
			...
			// 기반 스트림 fos 를 이용해 보조 스트림 bos 를 생성
			...
			try {
				...
				// 보조 스트림(bos) close() 시,
				// 보조 스트림 생성에 사용된 기반스트림 fos도 같이 close() 됨.
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void fileByInput() {
		...
		try {
			...
			while(true) {
				...
				sb.append((char)value); // char 로 강제 형변환 해서 글자형태로 sb 추가
			}
	...
	/**
	 * bufferedByteInput 보조 스트림을 이용한 성능 향상
	 * 
	 * InputStream 의 경우 외부 파일의 데이터를 읽어올 때 사용
	 * -> 읽어오는 파일의 크기가 작으면 보조스트림의
	 * 		성능 이점이 크게 눈에 띄지는 않음.
	 * 
	 */
	public void bufferedByteInput() {
	...
	public void readProperty() {
		...
		try {
			...
			// propertied.getProperties(key) : key 에 대응되는 value 반환
			System.out.println("username : " + properties.getProperty("username"));
			System.out.println("password : " + properties.getProperty("password"));
			System.out.println("language : " + properties.getProperty("language"));
			System.out.println("language : " + properties.getProperty("name"));
			// propertied.getProperties("")  // null 반환
	 ...
	 *  : FlieOutputStream + BufferdOutputStream
	 *  
	 *  + Scanner 대신 BufferedReader(문자열 입력용 스트림) 이용
	 *  -> BufferedReader 는 보조 스트림으로, 단독사용 불가능!
	 *  BufferedReader 생성시 기반스트림인 InputStreamReader -> InputStream 가 기반
	 *  
	 */
	public void fileCopy() {
		...
		try {
			...
			// 원본에서 읽어온 내욜 bytes 를 bos 를 이용해서 쓰기(출력)
			bos.write(bytes);
			bos.flush();  // 스트림에 남아있는 데이터를 모두 밀어내기
