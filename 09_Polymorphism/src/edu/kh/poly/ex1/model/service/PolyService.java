...
// 부모타입 참조변수 = 부모객체 주소
		...
		// tesla, spark 와 같은 자식객체의 고유필드, 메서드 사용 불가
	...
	// 매개 변수로 전달받은 정수 값에 따라 car. Tesla Spark 객체를 만들어
	// 반환하는 메서드
	...
	public void ex2() {
		...
		//Car[] arr = {new Car(), new Tesla(), new Spark90};
		...
		System.out.println(arr[1] instanceof Spark); // false (Spark 와 Tesla 관계 없음)
		System.out.println(arr[1] instanceof Car); // true (부모 -> 상속 관계)
	}
	// 다형성 중 다운캐스팅 확인 예제
	public void ex3() {
		// 다운 캐스팅이란?
		// 부모 타입 참조 변수가 자식 객체를 참조하는 기술 (업캐스팅_
		// 적용된 상태에서만 집행할 수 있다.
	...
	public void ex4() {
		...
		// 예외 처리
		if (c1 instanceof Spark) { // c1 이 Spark 이거나 상속관계 인가?
			// 맞다면
	...
	public void ex5() {
		...
		// 프로그램 "실행 전"
		// 컴파일러는 toString 메서드가 Car 에 있는 걸로 인식하여
		// c1.edu.kh.poly.ex1.model.dto.Car.toString () 메소드 코드를 연결
		// -> 정적 바인딩
		...
		//String edu.kh.poly.ex1.model.dto.Car.toString ()
		// 참조변수 c2 가 Car 타입이므로
		// toStirng() 호출 코드와 Car 의 toString 메소드 코드를 연결
		// -> 정적 바인딩
		
		// 실행해 보면 자식 (Spark)의 toString 이 호출되는것을 확인
		// 경차엔진 / 휘발유 / 4 / 0.5
		// -> 컴파일 시에는 부모(Car)와 바인딩 == 정적 바인딩
		// "실행시" 에는 정적 바인딩 보다 자식(Spark)에서 오버라이딩된
