...
public class User {
	...
	// 데이터 직접 접근 불가 원칙 -> 필드는 기본적으로 private(캡슐화 진행)
	...
	// 기본 생성자( -> 매개변수가 없는 생성자)
	public User() { // 기본 생성자 자동 완성 단축키 : ctrl + space + enter
	...
	public User(String userId, String userPw, String userName, int userAge, char userGender ) {
		...
		public User(String userId) {} // 매개변수의 갯수가 같은 생성자가 없음.
		
		public User(int userId ) {} // 매개변수의 개수는 같지만 타입이 다름
		
		public User(String userId, int userAge) {} // 매개변수의 개수는 같지만 하나의 타입이 다름

		public User(int userAge, String userId) {} // 매개변수의 개수, 타입은 같지만 순서가 다름
		
		//public User(int userAge, String userName) {} // 매개변수의 개수, 타입, 순서가 모두 같아서 불가
		// 변수명은 신경쓰지 않는다.
	...
	// 1. 만약에 오류가 있거나, 부모클래스에 없는 메서드를 잘못 작성했을 경우 컴파일 오류를 방지함
	// 2. 이 메서드가 부모클래스를 재정의 했다는 것을 한눈에 알 수 있음.
...
// toString() 메서드 : Object 최상위 메서드. 이미 만들어져 잇음
