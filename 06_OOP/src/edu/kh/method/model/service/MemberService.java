...
// 회원가입
	...
	public String login() {
		...
		// 회원 가입 정보(memberinfo 가 참조하는 Member 객체)에
		// 저장된 아이디, 비밀번호가
		...
		if(memberId.equals(memberInfo.getMemberId()) && memberPw.equals(memberInfo.getMemberPw())) {
			...
			//int[] arr = {1,2,3}
			//int[] newArr = arr; -> 주소값 복사
			
			// 회원가입 정보를 loginMember 에 대입하여
	...
	public int updateMember() {
		...
		int inputAge = sc.nextInt();
			// 2-2) 비밀번호를 입력받아 현재 로그인한 회원의 비밀번호와
		// 일치하는지 확인
		// -> 비밀번호가 일치하는 경우, 로그인한 회원의 이름과 나이 정보를
		// 입력받은 값으로 변경후 1이라는 값 을 반환
