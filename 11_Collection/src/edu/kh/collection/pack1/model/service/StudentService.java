...
public class StudentService {
	...
	//java.util.ArrayList : 배열형태 List (가장 대표적인 List 자식 클래스
	
	// new ArrayList() : 기본 생성자 -> 기본 크기 10 짜리 리스트 생성
	...
	public StudentService() {
		...
		studentList.add(new Student("오미나", 27, "충북 청주시", 'F', 70));
	...
	* 메뉴 출력용 메서드
	...
	public void displayMenu() {
		...
		do {
			...
			try { 
				...
				default : System.out.println("메뉴에 작성된 번호만 작성해 주세요");
	...
	public String addStudent() {
		...
		// UncheckedIOException : 강제 처리 X, 예외 발생하면 자동으로
		// 상위 호출 메서드로 예외가 전파됨
	 ...
	 * 삭제 시 
	 * "정말 삭제하시겠습니까? (Y/N) : "
	 * 'Y' 를 입력했다면 해당 인덱스에 있는 객체 삭제후 
	 * "000 님의 정보가 제거되었습니다" 리턴
	 * 'N' 을 입력했다면 "취소" 리턴
	 * 
	 * @return
	 */
		...
		// remove() 동작 시 뒤쪽 요소를 한칸씩 자동으로 당겨온다.
			 System.out.println("==========학생 정보 삭제=============");
			  ...
			  if(studentList.isEmpty()) {
				  ...
				  char ch = sc.next().toUpperCase().charAt(0); // 순서 중요
				  // "y" -> "Y" -> 'Y'
	...
	public void searchName1() {
		...
		for(Student std : studentList) {
			if(input.equals(std.getName())) { // 이름이 일치하는 경우
				System.out.println(std); // std.toStirng();
	...
	public void searchName2() {
		...
		for(Student std : studentList) {
			...
			if (std.getName().contains(input)) { // std.getName() 에 input 이 포함되어 있느냐?
				System.out.println(std);
				flag = false;
			}
			}
	 ...
	 * Student 에 Comparable 인터페이스 상속받아 오버라이딩한 compareTo()에
	 * 정의한 대로 정렬됨(나이 오름차순, 내림차순)
	 * 
	 * 방법 2. Comparator 클래스에 의한 정렬 compare() 사용(익명 내부 클래스 이용)
	 * 익명 내부 클래스 : 이름이 없는 슼래스를 즉석에서 선언해서 한 번만 사용할 목적으로 작성
	...
	public void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				// name 은 String 형이라 compareTo로 비교
				
				// Stirng.compareTo() : 자바에서 객체를 비교하는 메서드
				// (String이 Comaparble 을 상속받아 재정의 해둔 compareTo() 메서드를 이용하는것
